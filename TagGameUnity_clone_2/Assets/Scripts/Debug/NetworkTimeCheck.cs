...
namespace Tag.Debugging{
    ...
    public class NetworkTimeCheck : NetworkBehaviour
    {
        ...
        public override void OnNetworkSpawn(){
            ...
            // if(IsClient){
            //     NetworkManager.NetworkTimeSystem.LocalBufferSec = 0.1f;
            //     NetworkManager.NetworkTimeSystem.ServerBufferSec = 0.1f;
            // }
        ...
        private void OnTick()
        {
            ...
            // Debug.Log(localTime - serveTime);
            RequestDebugDisplayChannel.RaiseEvent($"localTime: {Math.Round(localTime, 4)}");
            RequestDebugDisplayChannel.RaiseEvent($"serveTime: {Math.Round(serveTime, 4)}");
            RequestDebugDisplayChannel.RaiseEvent($"Local server time diff: {(int)((localTime - serveTime)*1000)}");
            RequestDebugDisplayChannel.RaiseEvent($"localBufferSec: {Math.Round(localBufferSec, 4)}");
            RequestDebugDisplayChannel.RaiseEvent($"serverBufferSec: {Math.Round(serverBufferSec, 4)}");
        }
